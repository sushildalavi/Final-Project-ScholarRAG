...
- `Evaluation/` and `scripts/` directories present in repo root
...
python scripts/apply_doc_ids.py \
  ...
  --files \
    Evaluation/queries_120_master.json \
    Evaluation/retrieval_eval_cases.json \
    Evaluation/judge_eval_cases.json \
    Evaluation/msa_labeling_template.jsonl
```
...
path = pathlib.Path("Evaluation/retrieval_eval_cases.json")
...
cases_file = pathlib.Path("Evaluation/retrieval_eval_cases.json")
...
pathlib.Path("Evaluation/_retrieval_payload.json").write_text(json.dumps(payload))
print("Payload ready.")
EOF
...
curl -s -X POST http://localhost:8000/eval/run \
     ...
     -d @Evaluation/_retrieval_payload.json \
     -o Evaluation/retrieval_results.json
...
cases_file = pathlib.Path("Evaluation/judge_eval_cases.json")
cases_data = json.loads(cases_file.read_text())
...
pathlib.Path("Evaluation/_judge_payload.json").write_text(json.dumps(payload))
print("Payload ready.")
EOF
...
curl -s -X POST http://localhost:8000/eval/judge \
     -H "Content-Type: application/json" \
     -d @Evaluation/_judge_payload.json \
     -o Evaluation/judge_results.json
...
jq '.metrics' Evaluation/judge_results.json
...
python scripts/export_msa_records.py \
  Evaluation/judge_results.json \
  -o Evaluation/calibration_records_auto.jsonl

# Check label balance
jq -r '.label' Evaluation/calibration_records_auto.jsonl | sort | uniq -c
```
...
The file `Evaluation/msa_labeling_template.jsonl` contains 250 pre-structured annotation slots. Fill in the `sentence`, `evidence_text`, and `label` fields for each slot. `S` and `A` can be left `null`; the API will use default values (0.5).
...
```bash
cat Evaluation/calibration_records_auto.jsonl \
    Evaluation/msa_labeling_template_filled.jsonl \
    > Evaluation/calibration_records_merged.jsonl

wc -l Evaluation/calibration_records_merged.jsonl
```
...
records = [json.loads(l) for l in
           pathlib.Path("Evaluation/calibration_records_merged.jsonl").read_text().splitlines()
           if l.strip()]

payload = {"records": records}
pathlib.Path("Evaluation/_calibrate_payload.json").write_text(json.dumps(payload))
...
curl -s -X POST http://localhost:8000/confidence/calibrate \
     -H "Content-Type: application/json" \
     -d @Evaluation/_calibrate_payload.json \
     | jq .
```
...
| File | Purpose |
|------|---------|
| `Evaluation/queries_120_master.json` | Master dataset: 120 queries with full metadata, `_doc_key` identifiers, and null placeholder IDs |
| `Evaluation/retrieval_eval_cases.json` | 80 cases for `POST /eval/run` — cross-document retrieval evaluation |
| `Evaluation/judge_eval_cases.json` | 40 cases for `POST /eval/judge` — single-doc and multi-doc synthesis evaluation |
| `Evaluation/msa_labeling_template.jsonl` | 250 annotation slots for human-labeled M/S/A calibration data |
